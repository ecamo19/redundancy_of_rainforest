---
title: 'Construction of the raw-data set for the project Redundancy of reproductive traits'
...
abbr_change_salagado <- 
    data_salgado_traits %>% 
        select(-c(genero,especie, source, name_submitted)) %>%   
...
raw_data_vargas <- extract_tables("./raw_data/response_traits/original_raw_data_files/lista_arboles_sindromes_OVR05.pdf", 
...
data_vargas_traits <- 
    map_dfr(raw_data_vargas, as_tibble) %>% 
        ...
        # Create new colums
        unite(dispersal_syndrome_modified ,6:10, sep = "_") %>%  
        unite(pollination_syndrome_modified, 7:10, sep = "_") %>%   
        ...
        # Remove underscore from pins
         mutate(across(7, str_replace_all, pattern = "_", replacement = "")) %>% 
...
manual_input  <-   
    tribble(
         ~accepted_species, ~genero, ~especie, ~dispersal_syndrome_modified, ~pollination_syndrome_modified, ~sexual_system_modified, ~source,
        "Cecropia peltata", "Cecropia", "peltata","dani","dwin","ssdio","http://www.ecofog.gf/img/pdf/bibliographic_synthesis_ruth_tchana_thomas_monjoin_master_2_bioget_cecropia.pdf",
        "Tabernaemontana alba", "Tabernaemontana","alba","","","","",
        "Tabernaemontana arborea", "Tabernaemontana","arborea","","","",""
    )    
```
...
## Join reproductive traits with Wood density
...
raw_data_complete <- 
    ...
    rename(n_ind_wd = "n_ind")
```

```{r}
#write.csv(raw_data_complete,"./raw_data/raw_data_complete.csv")
```
...
```{r}
# Check TRY database
# This file was downloaded from TRY
data_trydb <- 
    read.delim("raw_data/response_traits/TryAccSpecies.txt", header = T) %>% 
        clean_names()
```

```{r}
# Read Species with SEED MASS in TRY
data_trydb_traits <-
    read.delim("raw_data/response_traits/species_traits.txt", header = T) %>% 
        clean_names()
...
# Only 84 species with seed mass
```

```{r}
# Check BIEN database
data_biendb <- read.csv("raw_data/data_from_bien_database_wet_forest.csv", 
                        header = T)
...
```{r}
anti_join(seed_mass_bien,seed_mass_try,  by= c("species" = "species", 
                                               "genus" = "genus"))
# 84 + 7 = 91
```